...
const Page = async ({ params }: { params: { agencyId: string } }) => {
  ...
  if (agencyDetails.connectAccountId) {
    ...
    currency = response.default_currency?.toUpperCase() || "SAR";
    ...
    totalPendingSesssions = checkoutSessions.data
      .filter((sessions) => sessions.status === "open")
      .map((session) => ({
        ...session,
        created: new Date(session.created).toLocaleDateString(),
        amount_total: session.amount_total ? session.amount_total / 100 : 0,
    ...
    potentialIncome = +totalPendingSesssions
      .reduce((total, session) => total + (session.amount_total || 0), 0)
      .toFixed(2);
          ...
          <Card className="flex-1 relative">
            <CardHeader>
              ...
              <CardTitle className="text-4xl">
                {net ? `${currency} ${net.toFixed(2)}` : `0.00 SAR`}
              </CardTitle>
