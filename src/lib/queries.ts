    ...
    create: {
      ...
      SidebarOption: {
        create: [
          ...
          //   name: 'Connections',
          //   icon: 'link',
          //   link: `/subaccount/${subAccount.id}/connections`,
          // },
          // {
          //   name: 'Workflows',
          //   icon: 'chip',
...
// export const getWorkflows = async (subacountId: string) => {
//   const editor = await db.workflows.findMany({
//     where: { subAccountId: subacountId },
//   })
...
export const upsertWorkflow = async (
  subaccountId: string,
  userId: string,
  workflow: z.infer<typeof CreateFlowFormSchema>,
  workflowId: string
) => {
  const response = await db.workflows.upsert({
    where: { id: workflowId },
    update: workflow,
    create: {
      ...workflow,
...
export const getWorkflow = async (subaccountId: string) => {
  const response = await db.workflows.findMany({
    where: { subAccountId: subaccountId },
  })
  return response
}